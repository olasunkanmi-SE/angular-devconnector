...
module.exports.likeOrUnlikeAPost = async (req, res, next) => {
  try {
    ...
    if (
      likes.filter((item) => item.user.toString() == req.user.id).length > 0
    ) {
    ...
    if (
      likes.filter((item) => item.user.toString() == req.user.id).length == 0 ||
      likes.filter((item) => item.user.toString() !== req.user.id)
    ) {
  ...
  try {
    ...
    comments.map((comment) => {
      if (comment.id == req.params.commentId) {
        ...
        if (
          likes.filter((like) => like.user.toString() == req.user.id).length > 0
        ) {
          const disLike = likes
            .map((like) => like.user.toString())
            .indexOf(req.user.id);
      ...
      if (comment.user.toString() == req.user.id) {
        ...
        replies.map((reply) => {
          if (reply.id === req.params.replyId) {
            let removeReply = comment.replies
              .map((item) => item.id)
